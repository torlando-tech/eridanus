...
class HubRoomManager {
    ...
    fun addGlobalBan(hash: ByteArray) { globalBans.add(hash.asKey()) }
    fun removeGlobalBan(hash: ByteArray) { globalBans.remove(hash.asKey()) }

    fun isGloballyBanned(hash: ByteArray?): Boolean {
        if (hash == null) return false
        return hash.asKey() in globalBans
    }

    fun globalBanList(): List<String> =
        globalBans.map { it.bytes.joinToString("") { b -> "%02x".format(b) } }.sorted()
